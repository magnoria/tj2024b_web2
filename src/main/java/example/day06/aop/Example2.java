...
class  security{
    ...
    // 지정한 메소드가 실행되기 전에 아래메소드가 실행 된다.
    // 자종헌 메소드는 'TestService2' 클래스의 *(모든메소드) 이면서 (..) 모든매개변수타입 *(모든리턴타입)
        // 즉] 'TestService2' 모든 메소드들은 실행되기전에 'securityCheck'(메소드가 자동으로 실행된다.'
    public void  securityCheck(){
        System.out.println("메소드 실행전 : [보안] 코로나 열체크");
    ...
    // 지정한 메소드는 'TestService2' 클래스의 'enter1'(특정메소드)이면서(..)모든매개변수타입, *(모든리턴타입)
        // 즉] 'TestService2' 의 enter1 메소드가 실행된 후에 'TestService2()'메소드가 *자동* 으로 실행된다
    public void  securityCheck2(){
        System.out.println("(함수 실행 후(after) :서비스 종료)");
    }

    // [3]
    @Before("execution(* example.day06.aop.TestService2.enter3(..)) && args(name)")
    // 지정한 메소드에서 name 매개변수를 가지고 와서 아래 메소드에서 사용이 가능하다.
    public void securityCheck3(String name){ // 로그처리, 보안검사 , 트랜잭션 등등 하기 위해서
        System.out.println("메소드 실행전(befoer) : " + name + "님이 코로나 검사 완료");

    }

    //[4]
    @AfterReturning(value = "execution(* example.day06.aop.TestService2.enter3(..))" , returning = "result")
    //지정한 메소드에가 정상적으로 실행 되었을때 result 리턴값을 가지고 와서 아래 메소드에서 사용이 가능하다.
    // 즉] 'TestService2' enter3 메소드가 실행후 return 값을 가지고 'securityCheck4' 실행
    public void securityCheck4(Object result){// 모든 매개변수들의 타입들을 대입하기 위해서
        System.out.println("메소드 실행 후(after) : " + result + " 가 결과값이 정상");
    }
    ...
    public Object timeCheck5( ProceedingJoinPoint joinPoint ) throws Throwable {
       ...
       //(*) 지정된 메소드('enter3') 실행
        ...
        //(2) 끝나는 시간
        long endTime = System.currentTimeMillis(); // 현재시간을 미리초(1/1000초) 반환

        //(3) 지정된 메소드가 실행했을때 걸린시간 계산
        long timeMs = endTime - startTime;
        //(4) 걸린시간 출력
        System.out.println("enter3 메소드가 실행에 걸린 시간 : " + timeMs + "ms" );

        //(5) 지정된 메소드가 리턴한 값 그대로 실행
        return result;
...
class  TestService2{
    ...
    public  void  enter1(){
        ...
        System.out.println("식당입장"); //비지니스 로직
    ...
    public void enter2(){
        // 부가기능 제외한 상태
        //new security().securityCheck(); //1. 일반적인 코드 재사용 방법
        System.out.println("학원 입장"); //비지니스 로직
    }

    //메소드3
    public boolean enter3(String name){

        System.out.println("헬스장입장"); //비지니스 로직
