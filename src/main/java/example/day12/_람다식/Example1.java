...
interface Calculator{ // 인터페이스 // 주로 추상메소드 (정의하지않고 선언부가 없는 함수)
    // 더하기 추상메소드 정의
...
public class Example1 { // 클래스
    ...
    public static void main(String[] args) {
        ...
        //2. 추상메소드 호출 , 구현이 필요하다. (1) implements (2)익명객체
            // - Calculator calc = new Calculator(); : 인터페이스는 객체 생성을 못한다. 생성자도 없고, 추상메소드이므로 구현만 했을뿐 실행이 불가
            // -> 이름 없는 객체로 추상화를 구현한다. -> 익명(객)체 , Calculator calc = new Calculator(){구현};
        //Calculator calc = new Calculator(){오버라이딩}; //<-----{} (1회성구현)객체
