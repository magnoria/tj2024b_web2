...
public class FileService {
    ...
    //[*] 업로드 된 파일을 저장할 포더 경로 , resources(폴더)
    // src -> main -> resources -> static -> upload
    // 1. 현재 프로젝트 최상위 디렉토리(폴더) 경로 찾기
    String baseDir = System.getProperty("user.dir"); // 현재 작업 최상위 폴더를 반환한다.
    // jetbrains://idea/navigate/reference?project=tj2024b_web2&path=~%5CIdeaProjects%5Ctj2024b_web2 , [PC마다 다르다]
    // 2. src 와 build 폴더 차이점 : src(실행전코드 : 개발자) vs build(실행후코드: 사용자)
    // * 개발자는 src에서 작업하고 항상 서버를 실행하므로써 작업한 코드를 build로 반영한다.(컴파일)
    // * 업로드는 개발자의 src에 업로드 하지 않고 배포/빌드 된 build 폴더에 업로드 한다.
    //String uploadPath = "/src/resources/static/upload/build/resources/main/static/upload/";

    String uploadPath = baseDir+"/build/resources/main/static/upload/";
    //->//idea/navigate/reference?project=tj2024b_web2/build/resources/main/static/upload/
    ...
    public String fileUpload( MultipartFile multipartFile ){ // (1) 매개변수로 'MultipartFile' 인터페이스 타입을 받는다.
        ...
        }catch (IOException e){ System.out.println(e);} // 만일 업로드 실패시 null 반환
    ...
    public boolean fileDelete(String filename){// (1) 삭제할 파일명을 매개변수로 받는다.
        String filePath = uploadPath + filename;//(2) 업로드 경로와 삭제할 파일명을 연결한다.
