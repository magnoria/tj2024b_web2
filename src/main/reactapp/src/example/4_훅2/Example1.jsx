import { useEffect, useState } from "react"
    ...
    export default function Example1(Props){
        ...
        //useEffect( 이펙트함수정의 , 의존성배열)
            //-> 차이점 : 모든 state가 변경될때 재렌더링/재실행 된다.
        useEffect(() => {
            console.log('이펙트 함수 실행')
        })//[]의존성배열이 없을때

        //(2) 의존성배열이 빈배열 일때 userEffect , 실행시점 1.탄생(최초실행) 2.X
        ...
        //(3) 의존성배열에 state 변수가 존재할때 useEffect, 실행시점 1.탄생(최초실행) 2.인생(재렌더링)
            //-> 차이점 : 의존성 배열에 존재하는 state 변수가 변경될때만 재실행이 된다. , 의존성 배열에 존재하지 않는 state변수가 변경될때는 재실행이 안된다.
        ...
        return (<>
                ...
                <button onClick={(e) => {setCount2(count2+1)}}>업데이트2 1번 useEffecttlfgod</button>
        </>)
    }
