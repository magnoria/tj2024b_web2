import axios from "axios"; // 임폴드 안되있었음
...
export default function Create( props ){
    ...
    const onValueChange = (e) => { // e: event 약어 : onChange 실행결과 정보를 매개변수로 받는다.
        //스프레드 연산자, {...기존객체, 새로운 속성명 : 새로운값}
        setProductForm({...productForm, [e.target.name] : e.target.value});

    }
    //[3] 제품등록버튼을 클릭했을때 , axios 이용하여 서버와 통신하고, navigate 이용하여 가상URL페이지로 전환한다.
    const navigate = useNavigate();
    const onCreate = async () => {
        const response = await axios.post('http://192.168.40.63:8080/day08/products' , productForm)
