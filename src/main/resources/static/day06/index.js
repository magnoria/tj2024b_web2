...
count = 1; // 변수값 수정
const count2 = 0; // 상수선언
//count2 = 1; //상수값 수정 -> 상수는 수정 불가 오류 Uncaught TypeError: Assignment to constant variable.




// * 주의할점 : 객체가 상수에 저장하고 객체내 속성값 수정은 가능하다.
const obj = { name : '유재석'}
obj.name = '강호동'
console.log(obj);
...
function fun1( param1, param2){}
    // 정의 2 : 익명함수 , 주로 변수/상수에 저장해서 사용한다. , 이벤트 함수
const fun2  = function(param1 , param2){}
    // 정의 3 : 화살표/람다식 함수 , 주로 변수/상수 저장해서 사용한다. , 이벤트 함수
...
//[3] 객체 :  {}중가로 객체: key와 value 으로 한쌍을 구성하여 여러쌍을 {}묶은 , [] 리스트 /배열
...
//[4] 백틱 : 문자열 템플릿, +연산자가 아닌 `(백팀)안에 ${}를 이용하여 문자열과 표현식을 연결할때 사용.
    // 벡틱 `${선언부 불가 / 호출부 가능/ 표현부가능}` , 
    // 선언부 : if/for/while/객체/함수/ 선언 등등 불가능,
...
console.log(name5)
console.log(age5)
//[6] 비구조화 할당과 나머지 연산자 , 배열 또는 객체 뒤에 ... 이용하여 나머지는 객체또는 배열로 저장하는 방법
...
// [8] 단축연산/평가 , 조건 && 참 일때 실팽 false 이면 실행이 안됨
        // 조건(A) && 참(B) : 만약에 조건이 true이면 b반환 false 이면 A  반환
        // 조건(A) || 거짓(B) : 만약에 조건이 true 이면 A반환 flase 이면 B 반환
console.log(loginState && '로그인상태'); // A가 true이라서 B가 반환
const loginState2 = false;
console.log( loginState2 && '로그인상태')// A 가 false 이라서 A가 반환

console.log(loginState || '비로그인상태');// A가 true 라서 A가 반환
